...
void solve() {
	ll i, j, n, m, x, r;
	cin >> x >> r >> m;
	double dx = (double)x / 60, dr = r, dm = m;

	//Optimal Max Distance
	// 1. Floor Value it can go
	double part1 = (int)(dm / (2 * dx));
	double remains = m - (part1 * (2 * dx));
	double part2 = remains / 2 * dx;
	double maxRunPossible = part1 + part2;
	deb3(part1, part2, maxRunPossible);
	if (maxRunPossible >= dr) {
		cout << "YES\n";
	} else {
		cout << "NO\n";
	}
