...
ll getAndScore(vl ip) {
	ll n = ip.size(), i, j, ans;
	for (i = 0; i < n; i++) {
		ans = 1;
		for (j = i; j < n; j++) {
			deb(ip[j]);
...
void generateArray(vl ip, ll i, ll n, ll ansSoFar, vector<bool> visited) {
	if (i == n) {
		ll j, ans = 1;
		for (j = 0 ; j < n; j++) {
			if (visited[j]) {
		...
		cout << "ans = " << (ll)ansSoFar << "\n";
		return;
	}
	generateArray(ip, i + 1, n, ansSoFar, visited);
	visited[i] = true;
	ansSoFar = (ll)((ll)ansSoFar & (ll)ip[i]);
	generateArray(ip, i + 1, n, ansSoFar, visited);
}

void solve() {
	ll i, j, n, m, x, index, newVal, q;
	cin >> n >> q;
	vl ip(n);
	fo(i, n) {
		cin >> ip[i];
	...
	generateArray(ip, 0, n, (ll)1, visited);
