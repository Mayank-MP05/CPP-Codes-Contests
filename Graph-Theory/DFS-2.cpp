...
bool isSafe(vector<string> ip, ll x, ll y) {
	ll n = ip.size();
	ll m = ip[0].size();
...
ll DFS(vector<string> ip, ll x, ll y, ll n, ll m) {
	if (isSafe(op, x, y)) {
		if (x == n - 1 and y == m - 1) {
			return true;
		}
		return DFS(ip, x + 1, y, n, m) or DFS(ip, x - 1, y, n, m) or DFS(ip, x, y + 1, n, m) or DFS(ip, x, y - 1, n, m);
	}
	return false;
}
